...
// Exibe os conjuntos FIRST e FOLLOW
    ...
    for (const nt of nonTerminals) {
        ...
        for (const t of terminals) {
            ...
            if (production === null || production === undefined) {
                ...
                const prodStr = production.length === 0 || production[0] === 'ε' ? 'ε' : production.join(' ');
...
function displayTrace(trace) {
    ...
    for (const step of trace) {
        ...
        html += '<tr class="' + (isError ? 'error-row' : isAccepted ? 'accepted-row' : '') + '">';
...
function generateInteractiveSentence() {
    ...
    stepsHtml += `<strong>Sentença gerada:</strong> <code>${generation.sentence || 'ε'}</code>`;
    ...
    // Coloca a sentença gerada no campo de entrada
    ...
    // Adiciona event listener ao botão de analisar sentença gerada
    if (generation.sentence) {
